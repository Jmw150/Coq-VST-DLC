...
public:
    ...
    raw_ostream& dump(raw_ostream& out, int ind) override
    {
        ...
        {
            NamedVar.second->dump(indent(out,
                                         ind) << NamedVar.first << ':', ind + 1);
        }
        Body->dump(indent(out, ind) << "Body:", ind + 1);
        return out;
    }
...
Value* BinaryExprAST::codegen()
{
    ...
    if(Op == '=')
    {
        ...
        VariableExprAST* LHSE = static_cast<VariableExprAST*>
                                (LHS.get());
        if(!LHSE)
        {
...
Value* IfExprAST::codegen()
{
    ...
    PHINode* PN = Builder->CreatePHI(Type::getDoubleTy(
                                         *TheContext), 2, "iftmp");
...
Value* ForExprAST::codegen()
{
    ...
    Value* CurVar = Builder->CreateLoad(Type::getDoubleTy(
                                            *TheContext), Alloca,
                                        VarName.c_str());
...
Function* FunctionAST::codegen()
{
    ...
    if(P.isBinaryOp())
    {
        BinopPrecedence[P.getOperatorName()] =
